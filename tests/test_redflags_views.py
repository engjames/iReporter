...
def test_for_updating_redflag_status_with_wrong_values():
    ...
    result6 = CLIENT().put('/api/v1/update-red-flags/1',
    ...
    assert json_data6['data'][0][
        'error-message'] == "the status must be a string. follow this example ->> {'status':'resolved'}"
...
def test_to_create_a_redflag_with_wrong_data():
    ...
    assert json_data1['data'][0][
        'error-message'] == "wrong body format. follow this example ->> {'createdBy':​James​, 'location':​​[12.4567,3.6789]​, 'comment': '​collapsed bridges'"
    assert json_data2['data'][0]['error-message'] == [
    ...
    assert json_data4['data'][0]['error-message'] == [
