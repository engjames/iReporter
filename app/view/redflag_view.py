...
class RedFlagViews(MethodView):
    def get(self, id):
        ...
        single_redflag_record = [
            redflag_record.__dict__ for redflag_record in redflag_list if redflag_record.__dict__['id'] == int(id)]
    ...
    def post(self):
        ...
        validate_data = Validators.validate_create_redflag(
            request.json['createdBy'], request.json['comment'])
        validate_location = Validators.validate_location(
        ...
        if redflag_list:
            check_for_existance = [redflag_record.__dict__ for redflag_record in redflag_list if redflag_record.__dict__['createdBy'] == request.json['createdBy']
                                   and redflag_record.__dict__['location'] == request.json['location'] and redflag_record.__dict__['comment'] == request.json['comment']]
        ...
        new_redflag_record = RedFlag(redflag_id, str(datetime.datetime.now(
        )), request.json['createdBy'], "red-flag", request.json['location'], "draft", request.json['comment'])
    ...
    def delete(self, id):
        ...
        if Validators.validate_redflag_id(id) is not True:
            return Validators.validate_redflag_id(id)
        redflag_exists = [
            flag for flag in redflag_list if flag.__dict__['id'] == int(id)]
        ...
        redflag_record = [
            record for record in redflag_list if record.__dict__['id'] == int(id)]
...
class UpdateStatus(MethodView):
    def put(self, id):
        ...
        if 'status' not in request.json:
            return jsonify({"status": 400, "data": [{"error-message": "wrong body format. follow this example ->> {'status':'under investigation'}"}]})
        elif not type(request.json['status']) in [str]:
        ...
        for redflag_record in redflag_list:
            if redflag_record.__dict__['id'] == int(id):
                ...
                redflag_record.__dict__[
                    'status'] = redflag_json['status'].lower()
...
class RedFlagUrls:
    ...
    def fetch_urls(app):
        ...
        app.add_url_rule(
            '/api/v1/red-flags', defaults={'id': None}, view_func=redflag_view, methods=['GET', ])
        app.add_url_rule('/api/v1/red-flags',
                         view_func=redflag_view, methods=['POST', ])
        app.add_url_rule('/api/v1/red-flags/<id>',
                         view_func=redflag_view,  methods=['GET', 'PUT', 'DELETE'])
        app.add_url_rule('/api/v1/update-red-flags/<id>',
                         view_func=update_status,  methods=['PUT'])
